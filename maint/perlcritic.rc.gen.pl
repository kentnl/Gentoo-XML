...
my @stopwords = (
  qw( Metadata metadata xml pkgmetadata longdescription changelog undef URI NodeSet
    accessors  metacollection subclasses metaobject accessor nodeset )
);
for my $wordlist (@stopwords) {
  $bundle->add_or_append_policy_field( 'Documentation::PodSpelling' => ( 'stop_words' => $wordlist ) );
}
$bundle->add_or_append_policy_field( 'TestingAndDebugging::ProhibitNoStrict'   => ( 'allow' => 'refs' ) );
$bundle->add_or_append_policy_field( 'TestingAndDebugging::ProhibitNoWarnings' => ( 'allow' => 'once' ) );
